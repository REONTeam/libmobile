...
//   preprocessor defines, or including a modified version of this header in
//   the relevant (buildtime?) paths.
// Make sure that both the library and any files that include the library
//   header are built with exactly the same options.

// MOBILE_LIBCONF_WEAK_IMPL - use weak implementation callbacks
//
...
// passing the callbacks as pointers into the mobile_register_* functions.
//
// This option should be kept disabled in any scenario where the library is
// built in shared mode. This is due to the ABI compatibility provided through
// the use of the mobile_register_* functions.
//
...
//
//#define MOBILE_LIBCONF_WEAK_IMPL
